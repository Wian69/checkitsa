...
export async function POST(request) {
    try {
        ...
        if (!cseKey || !cx) {
            return NextResponse.json({ valid: false, data: { status: 'Config Error', message: 'Intelligence services not configured.' } })
        }

        // 1. Enriched AI-First Search Query
        // We automatically include all the user's requested keywords to guide the web index and AI
        ...
        return NextResponse.json({
            valid: false,
            data: { status: 'Error', message: 'Unable to synthesize business intelligence at this time.' }
        })
